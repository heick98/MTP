	...
	void printImagem(Pixel * imagem, int qtd_pixels) {
		...
		for(i = 0; i < qtd_pixels; i++)
			printf("%04d : %02X %02X %02X : %03u %03u %03u\n", i, imagem[i].R, imagem[i].G, imagem[i].B, 
				imagem[i].G, imagem[i].B, imagem[i].R);
	}
	...
	Pixel * carregar_imagem(const char * nomearq, HeaderBMP24bpp * pheader) {
		...
		Pixel * salvar( HeaderBMP24bpp * pheader, Pixel * imagem) {
		int AUX, PAL, i, j;
		FILE * arq;
		arq = fopen("novaimagem.bmp", "wb");
		fwrite(pheader->identificador, sizeof(byte), 2, arq);
		fwrite(&(pheader->tam_arquivo), sizeof(dword), 1, arq);
		fwrite(&(pheader->infores1), sizeof(word), 1, arq);
		fwrite(&(pheader->infores2), sizeof(word), 1, arq);
		//fseek(arq, 2*sizeof(word), SEEK_CUR);
		fwrite(&(pheader->offset), sizeof(dword), 1, arq);
		fwrite(&(pheader->tam_cabecalho), sizeof(dword), 1, arq);
		fwrite(&(pheader->largura_px), sizeof(dword), 1, arq);
		fwrite(&(pheader->altura_px), sizeof(dword), 1, arq);
		fwrite(&(pheader->qtde_plano_cores), sizeof(word), 1, arq);
		fwrite(&(pheader->bits_per_pixel), sizeof(word), 1, arq);
		fwrite(&(pheader->metodo_compressao), sizeof(dword), 1, arq);
		fwrite(&(pheader->tam_imagem), sizeof(dword), 1, arq);
		fwrite(&(pheader->resolucao_horizontal), sizeof(dword), 1, arq);
		fwrite(&(pheader->resolucao_vertical), sizeof(dword), 1, arq);
		fwrite(&(pheader->numero_cores_paleta), sizeof(dword), 1, arq);
		if(pheader->numero_cores_paleta == 0)
			pheader->numero_cores_paleta = 1 << pheader->bits_per_pixel;
		fwrite(&(pheader->numero_cores_importantes), sizeof(dword), 1, arq);
		if(pheader->numero_cores_importantes == 0) 
			pheader->numero_cores_importantes = pheader->numero_cores_paleta;
		// *****************************************************************************
		//imagem = (Pixel *) calloc(sizeof(Pixel),pheader->largura_px*pheader->altura_px);
		PAL = floor((pheader->bits_per_pixel*pheader->largura_px + 31.)/32.)*4;
		AUX = PAL - pheader->largura_px*3;
	...
	int main() {
		...
		imagem = carregar_imagem("images.bmp", &header);
		salvar(&header, imagem);
		if(imagem) {
			printHeader(header);
